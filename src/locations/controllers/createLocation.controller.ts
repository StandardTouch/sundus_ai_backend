...
export async function createLocationController(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!createData.location_title || !createData.location_title_ara || !createData.location_address || !createData.location_address_ara || !createData.location_latitude || !createData.location_longitude) {
      res.status(400).json({
        success: false,
        error: "Missing required fields: location_title, location_title_ara, location_address, location_address_ara, location_latitude, location_longitude"
      });
      return;
    }
